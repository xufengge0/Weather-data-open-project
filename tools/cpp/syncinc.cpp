...
bool _syncinc(bool &bcontinue)
{
    ...
    stmtsel.prepare("select rowid from %s where %s>%d %s order by %s",starg.remotetname,starg.remotekeycol,maxkeyvalue,starg.where,starg.remotekeycol);
    char remrowidvalue[21];    // 存放从远程表查到的需要同步记录的键值。
    ...
    // insert into T_ZHOBTMIND3(stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid)
    //                   select obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid from T_ZHOBTMIND1@db128
    //                    where rowid in (:1,:2,:3,...,:starg.maxcount);
    ...
    for (int i = 0; i < starg.maxcount; i++)
    {
        stmtins.bindin(i+1,rowidvalues[i]);
    }
...
void _help(char *argv[])
{
    ...
    printf("Sample:/project/tools/bin/procctl 10 /project/tools/cpp/syncinc /log/idc/syncinc_ZHOBTMIND2.log "\
    ...
    printf("把T_ZHOBTMIND1@db128表中满足\"and obtid like '54%%'\"的记录增量同步到T_ZHOBTMIND3中。\n");
    printf("       /project/tools/bin/procctl 10 /project/tools/cpp/syncinc /log/idc/syncinc_ZHOBTMIND3.log "\
              "\"<localconnstr>idc/idcpwd@snorcl11g_128</localconnstr><remoteconnstr>idc/idcpwd@snorcl11g_128</remoteconnstr>"\
              "<charset>Simplified Chinese_China.AL32UTF8</charset>"\
              "<remotetname>T_ZHOBTMIND1</remotetname><lnktname>T_ZHOBTMIND1@db128</lnktname>"\
