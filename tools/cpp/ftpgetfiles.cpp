...
struct st_arg
{
    ...
    char remotepathbak[256];// 文件的备份目录
    char okfilename[256];   // 存储已下载文件名的文件
    bool checktime;         // 是否检查文件的修改时间,缺省为true
    int timeout;            // 心跳的超时时间
    char pname[51];             // 进程名
}starg;
...
int main(int argc,char* argv[])
{
    ...
    for (auto i:vdownload)
    {
        ...
        if(ftp.get(strremotefilename,strlocalfilename,starg.checktime)==false)
        {
        ...
        {   
            // 备份文件的全路径名
            string strremotefilebak = sformat("%s/%s",starg.remotepathbak,i.filename.c_str());
...
void _help()
{
    string s = R"(Example:./ftpgetfiles /log/idc/ftpgetfiles.log "<host>192.168.247.128:21</host><mode>1</mode>
...
<remotepathbak>/tmp/idc/surfdatabak</remotepathbak><okfilename>/home/mysql/client/okfilename.xml</okfilename>
<checktime>true</checktime><timeout>100</timeout><pname>ftpfileget</pname>")";
    s.erase(remove(s.begin(), s.end(), '\n'), s.end()); // 移除字符串中的换行符，remove将\n移到末尾，erase删除\n
    cout<<s<<endl;

    cout<<"本程序是通用的功能模块，用于从远程ftp服务器下载文件\n";
    cout<<"/log/idc/ftpgetfiles.log:为日志保存目录\n";
    ...
    cout<<"<remotepathbak>/tmp/idc/surfdatabak</remotepathbak>:文件的备份目录，ptype=3时生效\n";
    cout<<"<okfilename></okfilename>:存储已下载文件名的文件,ptype=1时生效\n";
    cout<<"<checktime>true</checktime>:是否检查文件的修改时间,缺省为true,ptype=1时生效\n";
